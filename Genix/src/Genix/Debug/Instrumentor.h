...
#define GX_PROFILE 1
	...
	// could mark the wrong one in your editor!
		...
		#define GX_FUNC_SIG __PRETTY_FUNCTION__
	#elif defined(__DMC__) && (__DMC__ >= 0x810)
		#define GX_FUNC_SIG __PRETTY_FUNCTION__
	#elif (defined(__FUNCSIG__) || (_MSC_VER))
		#define GX_FUNC_SIG __FUNCSIG__
	#elif (defined(__INTEL_COMPILER) && (__INTEL_COMPILER >= 600)) || (defined(__IBMCPP__) && (__IBMCPP__ >= 500))
		#define GX_FUNC_SIG __FUNCTION__
	#elif defined(__BORLANDC__) && (__BORLANDC__ >= 0x550)
		#define GX_FUNC_SIG __FUNC__
	#elif defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 199901)
		#define GX_FUNC_SIG __func__
	#elif defined(__cplusplus) && (__cplusplus >= 201103)
		#define GX_FUNC_SIG __func__
	#else
		#define GX_FUNC_SIG "GX_FUNC_SIG unknown!"
	#endif

	#define GX_PROFILE_BEGIN_SESSION(name, filepath) Instrumentor::Get().BeginSession(name, filepath)
	#define GX_PROFILE_END_SESSION() Instrumentor::Get().EndSession()
	#define GX_PROFILE_SCOPE_LINE2(name, line) constexpr auto fixedName##line = InstrumentorUtils::CleanupOutputString(name, "__cdecl ");\
		InstrumentationTimer timer##line(fixedName##line.Data)
	#define GX_PROFILE_SCOPE_LINE(name, line) GX_PROFILE_SCOPE_LINE2(name, line)
	#define GX_PROFILE_SCOPE(name) GX_PROFILE_SCOPE_LINE(name, __LINE__)
	#define GX_PROFILE_FUNCTION() GX_PROFILE_SCOPE(GX_FUNC_SIG)
#else
